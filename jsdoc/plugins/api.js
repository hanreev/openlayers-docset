...
function includeAugments(doclet) {
  ...
  if (cls.observables && !doclet.observables) doclet.observables = cls.observables;

  if (doclet.fires && cls.fires)
    for (let i = 0, ii = cls.fires.length; i < ii; ++i) {
      const fires = cls.fires[i];
      if (doclet.fires.indexOf(fires) == -1) doclet.fires.push(fires);
    }

  if (cls.fires && !doclet.fires) doclet.fires = cls.fires;
...
// @ts-ignore
const moduleRoot = env.conf.typescript.moduleRoot;
